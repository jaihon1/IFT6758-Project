...
### Strength Feature
Looking more deeper into the API, we can observe that the "strength" feature is not always available. Only the "Goal" event has a property strength. In the future, there are couple of ways we could associate the strength feature with the "Shot" event.
...
Another feature could be goals or chances created by a team on the counter-attack. We could generate these features using the raw data in the following way. We could look at when a goalie saves the puck or when a defending player blocks a shot, and then we would look if there is another event right after this one that leads to a goal in the other end of the rink (In a predefined time window of 5-10 seconds).
