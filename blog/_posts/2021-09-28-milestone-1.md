...
The class also have the load method, the add_season method, and the __add__ method that we added
in the class to be able to load the json files already downloaded, to add games from a new season and to use the sum
operator to combine two GamesInfo together. The __add__ method returns a new GamesInfo object which contains all of the
